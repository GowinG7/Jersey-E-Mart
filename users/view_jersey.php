<?php
require_once "../shared/dbconnect.php";
include_once "../shared/commonlinks.php";

if (!isset($_GET['id']))
    die("Invalid product request.");

$id = intval($_GET['id']);
$result = mysqli_query($conn, "SELECT * FROM products WHERE id=$id LIMIT 1");

if (!$result || mysqli_num_rows($result) == 0)
    die("Product not found.");

$p = mysqli_fetch_assoc($result);
$img = "../shared/products/" . $p['image'];
?>
<!DOCTYPE html>
<html>

<head>
    <title><?php echo htmlspecialchars($p['j_name']); ?></title>

    <style>
        .size-btn {
            padding: 8px 18px;
            border: 1px solid #ccc;
            border-radius: 6px;
            background: #fff;
            cursor: pointer;
            margin: 4px;
        }

        .size-btn:hover {
            border-color: #1c6059;
        }

        .size-btn.active {
            background: #1c6059;
            color: white;
            border-color: #1c6059;
        }

        .qty-box {
            width: 70px;
        }

        .note-box {
            background: #fff8e6;
            border-left: 4px solid #ffc107;
            padding: 10px 12px;
            font-size: 14px;
            border-radius: 6px;
            margin-top: 15px;
        }
    </style>

    <script>
        function selectSize(btn) {
            document.querySelectorAll(".size-btn").forEach(b => b.classList.remove("active"));
            btn.classList.add("active");

            document.getElementById("sizeInput").value = btn.dataset.size;
        }
    </script>
</head>

<body style="background:#e9f8f6; font-family:Segoe UI;">
    <div class="container py-5">

        <div class="row g-4">

            <!-- LEFT IMAGE -->
            <div class="col-md-6 text-center">
                <img src="<?php echo $img; ?>" class="img-fluid shadow-sm rounded"
                    style="background:#fff;padding:18px;max-height:480px;object-fit:contain;">
            </div>

            <!-- RIGHT DETAILS -->
            <div class="col-md-6">

                <h2 class="fw-bold" style="color:#1c6059;"><?php echo htmlspecialchars($p['j_name']); ?></h2>

                <p class="text-muted"><?php echo htmlspecialchars($p['description']); ?></p>

                <!-- PRICE -->
                <div class="my-3">
                    <?php if ($p['discount'] > 0) {
                        $new = $p['price'] - ($p['price'] * $p['discount'] / 100); ?>

                        <span style="text-decoration:line-through;color:#777;">
                            Rs <?php echo number_format($p['price'], 0); ?>
                        </span>

                        <span class="fw-bold text-success h4 ms-2">
                            Rs <?php echo number_format($new, 0); ?>
                        </span>

                        <span class="badge bg-danger ms-2">
                            <?php echo $p['discount']; ?>% OFF
                        </span>

                    <?php } else { ?>
                        <h4 class="text-success fw-bold">
                            Rs <?php echo number_format($p['price'], 0); ?>
                        </h4>
                    <?php } ?>
                </div>

                <!-- SIZE SELECTION -->
                <h6 class="fw-semibold">Available Sizes</h6>

                <div class="d-flex flex-wrap mb-3">
                    <?php
                    $sizes = mysqli_query(
                        $conn,
                        "SELECT size,stock FROM product_sizes 
                         WHERE product_id=$id AND stock>0 
                         ORDER BY size ASC"
                    );

                    if (mysqli_num_rows($sizes)) {
                        while ($s = mysqli_fetch_assoc($sizes)) {
                            $size = strtoupper($s['size']);
                            $stock = $s['stock'];

                            echo "<button type='button' 
                                        class='size-btn' 
                                        data-size='$size' 
                                        onclick='selectSize(this)'>
                                        $size • In stock: $stock
                                   </button>";
                        }
                    } else {
                        echo "<span class='text-danger'>No sizes available</span>";
                    }
                    ?>
                </div>

                <input type="hidden" id="sizeInput">

                <!-- TRANSPARENCY NOTICE -->
                <div class="note-box">
                    If you need a complete jersey set (full kit), you can contact the admin for better prices or offers.
                </div>

                <!-- QUANTITY -->
                <label class="fw-semibold mt-3">Quantity</label>
                <input type="number" class="form-control qty-box mb-3" value="1" min="1">

                <!-- BUTTONS -->
                <div class="d-flex mt-3">
                    <a href="add_to_cart.php?id=<?php echo $id; ?>" class="btn btn-danger px-4 me-3">
                        Add to Cart
                    </a>

                    <button class="btn btn-dark px-4" data-bs-toggle="modal" data-bs-target="#customizeModal">
                        Customize
                    </button>


                </div>

            </div>
        </div>
    </div>


    <!-- CUSTOMIZE MODAL -->
    <div class="modal fade" id="customizeModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-3">

                <div class="modal-header">
                    <h5 class="modal-title fw-bold">Customize Jersey</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>

                <form action="save_custom_cart.php" method="POST">

                    <input type="hidden" name="product_id" value="<?php echo $p['id']; ?>">
                    <input type="hidden" id="finalPrice" name="final_price" value="<?php echo $p['price']; ?>">

                    <div class="modal-body">

                        <!-- PRICE DISPLAY -->
                        <div class="mb-3">
                            <label class="fw-semibold">Final Price:</label>
                            <h4 class="text-success fw-bold" id="priceDisplay">
                                Rs <?php echo number_format($p['price'], 0); ?>
                            </h4>
                        </div>

                        <!-- TYPE -->
                        <label class="form-label fw-semibold">Jersey Type:</label>
                        <select class="form-select" id="jerseyType" name="type" required>
                            <option value="">Select Type</option>
                            <option value="First Copy">First Copy</option>
                            <option value="Premium">Premium (+1000)</option>
                            <option value="Replica">Replica (-700)</option>
                        </select>

                        <!-- NAME -->
                        <label class="form-label fw-semibold mt-3">Name to Print (optional):</label>
                        <input type="text" class="form-control" name="print_name">

                        <!-- NUMBER -->
                        <label class="form-label fw-semibold mt-3">Number to Print (optional):</label>
                        <input type="number" class="form-control" name="print_number" min="0" max="99">


                    </div>

                    <div class="modal-footer">
                        <button type="submit" class="btn btn-dark w-100 fw-semibold">Add to Cart</button>
                    </div>

                </form>

            </div>
        </div>
    </div>


    <script>
        const basePrice = <?php echo $p['price']; ?>;

        const priceDisplay = document.getElementById("priceDisplay");
        const finalPriceInput = document.getElementById("finalPrice");

        const typeSelect = document.getElementById("jerseyType");
        const nameInput = document.querySelector("input[name='print_name']");
        const numberInput = document.querySelector("input[name='print_number']");

        function updatePrice() {
            let finalPrice = basePrice;

            // Jersey Type Adjustment
            let type = typeSelect.value;
            if (type === "Premium") finalPrice += 1000;
            if (type === "Replica") finalPrice -= 700;

            // Printing Costs
            let name = nameInput.value.trim();
            let number = numberInput.value.trim();

            if (name !== "" && number !== "") {
                finalPrice += 150;      // Name + Number
            } else if (name !== "") {
                finalPrice += 150;      // Only Name
            } else if (number !== "") {
                finalPrice += 100;      // Only Number
            }

            // Round to whole number
            finalPrice = Math.round(finalPrice);

            // Format with commas for thousands
            priceDisplay.innerText = "Rs " + finalPrice;
            finalPriceInput.value = finalPrice;

        }

        typeSelect.addEventListener("change", updatePrice);
        nameInput.addEventListener("input", updatePrice);
        numberInput.addEventListener("input", updatePrice);
    </script>



</body>

</html>